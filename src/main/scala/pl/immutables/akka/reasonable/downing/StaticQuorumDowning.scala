...
case class StaticQuorumDowningSettings(quorum: Int, seed: Int, stableAfter: FiniteDuration)
...
object StaticQuorumDowningSettings {
  def apply(conf: Config): StaticQuorumDowningSettings = StaticQuorumDowningSettings(
    ...
    conf.getInt("akka.reasonable.downing.min-seed-size"),
    FiniteDuration(conf.getDuration("akka.reasonable.downing.stable-after").toMillis,
      TimeUnit.MILLISECONDS)
  )
}

class StaticQuorumDowning(cluster: Cluster, settings: StaticQuorumDowningSettings)(implicit ex: ExecutionContext)
  ...
  def isSeedDown = cluster.state.members.filter(_.address.host.get.contains("seed")).isEmpty
